...
globalThis.getVariables=opts=>({stat_data:ctx.chat[opts.message_id]?.stat_data});
globalThis.updateVariablesWith=(fn,opts)=>{const index=opts.message_id;const next=fn({stat_data:ctx.chat[index].stat_data});ctx.chat[index].stat_data=next.stat_data;};
...
function setup(){
 ...
 const before={主角:{载体档案:{姓名:'旧身体'},换身状态:{阶段:'等待确认',当前身体死亡已确认:true,当前世代编号:1,待确认人物卡:card,当前身体死亡信息:'已死亡',待归档人生词条:'',归档写入状态:'无待处理'}},历代记忆摘要:[]};
 const after={主角:{载体档案:{姓名:'新身体'},换身状态:{阶段:'当前身体生效',当前身体死亡已确认:false,当前世代编号:2,待确认人物卡:'',当前身体死亡信息:'',待归档人生词条:draft,归档写入状态:'待写入世界书'}},历代记忆摘要:[{世代编号:1,身体姓名:'旧身体',详细词条名称:'第1世·旧身体'}]};
...
test('真实正文生成前两插件共同暂停确认首轮，普通下一轮恢复档案',()=>{
 const after=setup();const body={profile:{姓名:'新身体'},rawCard:card,text:card};assert.equal(plugin.buildCurrentBodyPrompt(body,after,'compact').prompt,'');ctx.chat.push({is_user:true,mes:'观察门口'});assert.match(plugin.buildCurrentBodyPrompt(body,after,'compact').prompt,/<legacy_life_current_body>/);
});
